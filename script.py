...
if parser == 'DG':
    ...
    jar_path = '~/DG_HPSG_NER/stanford-corenlp-4.2.2/stanford-corenlp-4.2.2.jar'
    models_jar_path = '~/DG_HPSG_NER/stanford-corenlp-4.2.2-models-english.jar'
    special_tokens = ['[START]','[SEP]', '[TAG]', '[END]', '[UNK]', '[PAD]']
    max_len = 150

    dg = DependencyGrammar(jar_path, models_jar_path, path, language, special_tokens, max_len, parser)
    dg.run()
